#ifndef VOXBLOX_INTEGRATOR_NP_TSDF_INTEGRATOR_H_
#define VOXBLOX_INTEGRATOR_NP_TSDF_INTEGRATOR_H_
 ...
 */
class NpTsdfIntegratorBase {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  typedef std::shared_ptr<NpTsdfIntegratorBase> Ptr;
  ...
  struct Config {
    ...
    float reliable_band_ratio = 2.0f;
    bool curve_assumption = false;
    float reliable_normal_ratio_thre = 0.1f;
    ...
    // only for "merge" integration
    bool merge_with_clear = true;
  ...
  NpTsdfIntegratorBase(const Config& config, Layer<TsdfVoxel>* layer);
 ...
 protected:
  ...
  inline bool isPointValid(
      const Point& point_C, const bool freespace_point,
  ...
  void updateTsdfVoxelValue(
      ...
      const Color* color = nullptr) const;  // NOLINT
...
class NpTsdfIntegratorFactory {
 public:
  static NpTsdfIntegratorBase::Ptr create(
 ...
 */
class SimpleNpTsdfIntegrator : public NpTsdfIntegratorBase {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  SimpleNpTsdfIntegrator(const Config& config, Layer<TsdfVoxel>* layer)
 ...
 */
class MergedNpTsdfIntegrator : public NpTsdfIntegratorBase {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  MergedNpTsdfIntegrator(const Config& config, Layer<TsdfVoxel>* layer)
      : NpTsdfIntegratorBase(config, layer) {}
 ...
 * for small voxels.
 */
class FastNpTsdfIntegrator : public NpTsdfIntegratorBase {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  FastNpTsdfIntegrator(const Config& config, Layer<TsdfVoxel>* layer)
      : NpTsdfIntegratorBase(config, layer) {}
...
#endif  // VOXBLOX_INTEGRATOR_NP_TSDF_INTEGRATOR_H_
