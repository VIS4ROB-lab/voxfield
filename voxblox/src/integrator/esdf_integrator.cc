...
void EsdfIntegrator::addNewRobotPosition(const Point& position) {
  timing::Timer clear_timer("upate_esdf/voxblox/clear_radius");
...
void EsdfIntegrator::updateFromTsdfBlocks(const BlockIndexList& tsdf_blocks,
  ...
  size_t num_raise = 0u;
  size_t num_new = 0u;
  timing::Timer propagate_timer("upate_esdf/voxblox/propagate_tsdf");
  ...
  timing::Timer raise_timer("upate_esdf/voxblox/raise");
  processRaiseSet();
  raise_timer.Stop();

  timing::Timer update_timer("upate_esdf/voxblox/update");
  processOpenSet();
  update_timer.Stop();
